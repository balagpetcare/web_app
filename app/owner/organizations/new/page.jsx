...
export default function NewOrganizationPage() {
  ...
  async function ensureOrgCreated(state) {
    ...
    const created = await apiPost("/api/v1/owner/organizations", {
      ...
      fullPathText: state?.location?.fullPathText || state?.location?.formattedAddress || state?.location?.text || null,
      addressJson: {
        ...
        countryName: state?.location?.countryName || null,
        stateName: state?.location?.stateName || null,
        cityName: state?.location?.cityName || null,
        postalCode: state?.location?.postalCode || null,
        addressLine: state?.location?.addressLine || null,
        formattedAddress: state?.location?.formattedAddress || null,
        provider: state?.location?.provider || null,
        providerPlaceId: state?.location?.providerPlaceId || null,
        cityCorporationId: state?.location?.cityCorporationId || null,
